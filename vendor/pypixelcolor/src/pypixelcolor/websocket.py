...
# Optional shared-secret token (review F-1). When set, every WebSocket connection
# must present it as `Authorization: Bearer <token>`. The server still binds
# loopback by default; this is defence-in-depth for any non-loopback exposure.
_auth_token = None


def _authorized(websocket) -> bool:
    """Constant-time check of the Authorization header against the shared token."""
    if not _auth_token:
        return True  # auth disabled
    # websockets exposes request headers differently across versions.
    ...
    if not value.lower().startswith(prefix):
        return False
    return hmac.compare_digest(value[len(prefix):].strip(), _auth_token)
...
async def handle_websocket(websocket):
    ...
    # Reject unauthenticated clients when a token is configured (F-1).
...
async def start_server(ip, port, address, token=None):
    """Start the WebSocket server and maintain BLE connection."""
    global _device_session, _auth_token

    _auth_token = token or os.environ.get("IPIXEL_WS_TOKEN")
    if _auth_token:
        logger.info("WebSocket auth enabled (Authorization: Bearer required)")
    elif ip not in ("localhost", "127.0.0.1", "::1"):
        logger.warning(
            "Binding %s with NO auth token — anyone who can reach this port can "
            "control the device. Set --token / IPIXEL_WS_TOKEN, or bind loopback.",
            ip,
        )
...
if __name__ == "__main__":    
    ...
    parser.add_argument("--host", default="localhost", help="Bind address (default localhost; only expose beyond loopback behind a token + network ACL)")
    parser.add_argument("-a", "--address", required=True, help="Specify the Bluetooth device address")
    parser.add_argument("--token", default=None, help="Shared-secret bearer token required from clients (or set IPIXEL_WS_TOKEN). Strongly recommended for any non-loopback bind.")
    ...
    asyncio.run(start_server(args.host, args.port, args.address, token=args.token))
