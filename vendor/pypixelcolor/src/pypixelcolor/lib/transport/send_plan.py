...
# Smallest usable ATT payload (default MTU 23 - 3 byte ATT header).
_MIN_CHUNK = 20


def effective_chunk_size(client, plan) -> int:
    """Chunk size that respects the *negotiated* ATT MTU (review H-MTU).

    The plan defaults ``chunk_size`` to 244 (assumes a 247-byte MTU). If the link
    actually negotiated a smaller MTU, writing 244-byte chunks silently overflows
    and the device garbles the transfer. Cap the chunk at ``mtu - 3`` so a
    degraded link slows down instead of corrupting. When the MTU is unknown
    (e.g. a non-Bleak fake in tests) fall back to the plan's chunk size.
    """
    mtu = getattr(client, "mtu_size", None)
    if not isinstance(mtu, int) or mtu <= 0:
        return plan.chunk_size
    return max(_MIN_CHUNK, min(plan.chunk_size, mtu - 3))
...
async def send_plan(client: BleakClient, plan: SendPlan, ack_mgr: AckManager, *, write_uuid: str = WRITE_UUID, ack_timeout: float = 8.0) -> CommandResult:
    ...
    # Chunk by the negotiated MTU, not the hardcoded 244 (review H-MTU).
    chunk_size = effective_chunk_size(client, plan)
    if chunk_size < plan.chunk_size:
        logger.warning(
            "ATT MTU limits chunk size to %d (plan wanted %d); transfer will be slower",
    ...
    try:
        ...
        for idx, win in enumerate(plan.windows):
            ...
            for chunk in _chunk_bytes(win.data, chunk_size):
