...
# SECURITY (review F-3): decompression-bomb / resource-exhaustion limits.
# Pillow's default DecompressionBombWarning (~89 Mpx) is only a warning. We set a
# finite, overridable cap and treat it as an error, and we cap GIF frame counts
# so a crafted animation can't exhaust memory/CPU. These are deliberately
# generous (real phone photos pass) but bounded.
MAX_IMAGE_PIXELS = 50_000_000          # ~50 Mpx hard ceiling for a single frame
MAX_GIF_FRAMES = 1024                   # reject pathological frame counts
Image.MAX_IMAGE_PIXELS = MAX_IMAGE_PIXELS  # make Pillow raise DecompressionBombError


def _open_image_guarded(file_bytes: bytes) -> "Image.Image":
    """Open untrusted image bytes with bomb + frame-count guards (F-3).

    Raises ValueError on an oversized image or an over-long animation. The caller
    owns the returned image and should close it.
    """
    try:
        img = Image.open(BytesIO(file_bytes))
    except Image.DecompressionBombError as exc:
        raise ValueError("image exceeds the maximum allowed pixel count") from exc
    w, h = img.size
    if w * h > MAX_IMAGE_PIXELS:
        img.close()
        raise ValueError(
            f"image {w}x{h} exceeds the {MAX_IMAGE_PIXELS}-pixel limit"
        )
    n_frames = getattr(img, "n_frames", 1)
    if n_frames > MAX_GIF_FRAMES:
        img.close()
        raise ValueError(
            f"animation has {n_frames} frames; the limit is {MAX_GIF_FRAMES}"
        )
    return img
...
def _resize_image(file_bytes: bytes, is_gif: bool, target_width: int, target_height: int, fit_mode: ResizeMethod = ResizeMethod.CROP) -> bytes:
    ...
    """
    img = _open_image_guarded(file_bytes)
...
def _process_loaded_bytes(file_bytes: bytes, extension: str) -> tuple[bytes, bool]:
    ...
    if ext in [".webp", ".jpg", ".jpeg", ".bmp", ".tiff", ".heic", ".heif"]:
        ...
        # Guarded open + explicit close (F-3 bomb guard, F-11 handle leak).
        with _open_image_guarded(file_bytes) as img:
            output = BytesIO()
