...
def send_text(text: str,
    ...
    # SECURITY (review F-6): the glyph/chunk count is stored in a single byte, so
    # it must be < 256. In var-width mode an emoji-heavy or long string can yield
    # more chunks than the 1..500 text-length check allows, and fixed-width text
    # over 255 chars also overflowed `bytes([num_chars])` with an opaque
    # ValueError. Fail with a clear, bounded message instead.
    if not (0 < num_chars <= 255):
        raise ValueError(
            f"Encoded glyph count {num_chars} exceeds the device limit of 255; "
            "send shorter text."
        )
